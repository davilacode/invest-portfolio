    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-200 dark:from-neutral-900 dark:to-neutral-950 px-4 py-10">
      <div className="w-full max-w-md">
        <div className="bg-white/70 dark:bg-neutral-900/60 backdrop-blur shadow-xl rounded-2xl p-8 border border-slate-200/60 dark:border-neutral-800">
          <h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100 mb-6 text-center">Registro</h2>
          <form onSubmit={onSubmit} className="space-y-5" noValidate>
            <div className="space-y-1">
              ...
              <input
                id="username"
                className="w-full rounded-lg border border-slate-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-2 text-sm dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                value={username}
            ...
            >
              ...
              <span className={loading ? 'pl-4' : ''}>{loading ? 'Enviando…' : 'Crear cuenta'}</span>
            </button>
          </form>
          <p className="mt-6 text-center text-xs text-slate-600 dark:text-slate-400">
            ¿Ya tienes cuenta?{' '}
            <Link to="/login" className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 underline decoration-indigo-400/40 hover:decoration-indigo-400">Inicia sesión</Link>
          </p>
        </div>
      </div>
    </div>
